    ...
    <main className="pb-32" id="results">
      ...
      <section className="py-12" aria-labelledby="charts-title">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="h-[320px] border border-[#262626] bg-[#111111] p-4"><p className="data-label">Factor radar</p><ResponsiveContainer width="100%" height="92%"><RadarChart data={factorData}><PolarGrid stroke="#2E4540" /><PolarAngleAxis dataKey="name" tick={{ fill: "#B5B9F0", fontSize: 10 }} /><PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} /><Radar dataKey="score" stroke="#B5B9F0" fill="#408175" fillOpacity={0.42} /></RadarChart></ResponsiveContainer></div>
          <div className="h-[320px] border border-[#262626] bg-[#111111] p-4"><p className="data-label">Weighted contribution</p><ResponsiveContainer width="100%" height="92%"><BarChart data={factorData} margin={{ top: 16, right: 4, bottom: 0, left: -24 }}><CartesianGrid stroke="#2E4540" vertical={false} /><XAxis dataKey="name" tick={{ fill: "#B5B9F0", fontSize: 10 }} tickLine={false} axisLine={false} interval={0} /><YAxis tick={{ fill: "#B5B9F0", fontSize: 10 }} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ background: "#2E4540", border: "1px solid #408175", color: "#B5B9F0" }} cursor={{ fill: "#2E4540" }} /><Bar dataKey="contribution" radius={0}>{factorData.map((factor) => <Cell key={factor.name} fill="#408175" />)}</Bar></BarChart></ResponsiveContainer></div>
        </div>
      </section>
